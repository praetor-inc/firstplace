...
import { filterListings } from '@/lib/data';
...
function SearchContent() {
    ...
    const [filters, setFilters] = useState({ ...DEFAULT_FILTERS, searchQuery: queryParam });
    ...
    useEffect(() => {
        async function loadListings() {
            ...
            if (error) {
                ...
                setFetchedListings(data as Listing[]);
            }
            setIsLoading(false);
        }
        loadListings();
    ...
    const listings = useMemo(() => filterListings(fetchedListings, filters), [fetchedListings, filters]);
    ...
    return (
        <div className={styles.app}>
            ...
            <header className={styles.header}>
                ...
                {/* Toggle filters button right next to search form so it expands logically from the left */}
                <button
                    className={`${styles.filterToggleBtn} btn ${showFilters ? 'btn-secondary' : 'btn-primary'} btn-sm`}
            ...
            <main className={`${styles.main} ${view === 'list' ? styles.mainList : view === 'map' ? styles.mainMap : styles.mainSplit}`}>
                {/* Filters panel - anchored to the left under the toggle */}
                <div
                    id="filters-panel"
                    ...
                    <div className={styles.listingsPanel} ref={listRef}>
                        ...
                        {isLoading ? (
                            <div className={styles.emptyState}>
                                ...
                                <p>Fetching fresh student rentals directly from the database.</p>
                            </div>
                        ) : listings.length === 0 ? (
                            <div className={styles.emptyState}>
                                <span className={styles.emptyEmoji}>🏠</span>
                                ...
                                {listings.map(listing => (
                                    <div key={listing.id} id={`card-${listing.id}`}>
                                        <ListingCard
                                            ...
                                            compact={true} // ensure list cards are a bit smaller for dense view
                                        />
                                    </div>
                                ))}
                            </div>
...
export default function SearchPage() {
    return (
        <Suspense fallback={<div style={{ padding: '60px', textAlign: 'center', fontFamily: 'var(--font-heading)', background: 'var(--paper)', minHeight: '100vh' }}>Loading FirstPlace...</div>}>
            <SearchContent />
        </Suspense>
    )
}
